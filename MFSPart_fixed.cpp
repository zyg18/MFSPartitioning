...
int random_choice(std::vector<std::pair<int, double>>&gain, std::unordered_set<int>&exclude_fpga, double total_edge_weight, int T=0){
    ...
    for (const auto& pair : gain){
        ...
        /*
        if(T == 1)
            std::cout<<"probability: "<<exp_gain/sum_exp_gain<<" fpga_id: "<< pair.first <<" ";*/
...
{  
    ...
    while(!fpga_heaps.empty()){
        ...
        for (int fpga_id : CP[node_id]){
            ...
            /*
            if(num<10)
                std::cout<<"fpga_id:"<<fpga_id<<" "<<edge_weight_in_fpga_per_node[node_id][fpga_id]<<" "<<1e4/(capacity+1-node_weight[node_id]-nodes_fixed_in_fpga[fpga_id])<<std::endl;*/
        ...
        for (int i=0;i<fpga_size;i++){
            ...
            if(num<10)
                T = 1;
            best_fpga = random_choice(gain, exclude_fpga, total_edge_weight, T);
            /*
            if(num<10){
...
int main(int argc, char* argv[]) {
    ...
    uncoarsen_CP = FM(current_graph, current_fixed_nodes, uncoarsen_CP, fpga_distances, current_node_weight, beta, capacity, speedup, data);
    ...
    for (int i=coarsen_level-1;i>=0;i--){
        ...
        uncoarsen_CP = FM(graph_previous_level, fixed_node_previous_level, CP_previous_level, fpga_distances, node_weight_previous_level, beta, capacity, speedup, data);
        ...
        if(i<5){
            ...
            for(int ii=0;ii<nodenum;ii++){
                int fpga_id = *uncoarsen_CP[ii].begin();
                for (CSRMatrix::InnerIterator it(graph_previous_level, ii); it; ++it) {
                    int neighbor_id = it.index();
                    if (neighbor_id < ii)
                        continue;
