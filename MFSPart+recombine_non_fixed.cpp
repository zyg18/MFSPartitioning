...
std::vector<std::vector<int>> crossover(const CSRMatrix &current_graph, std::vector<std::vector<int>> CP_sets, const std::vector<std::vector<int>> &fpga_distances, const CSRMatrix& fpga_matrix,  const std::vector<int> &current_node_weight, const int  capacity){
    ...
    for(int i = 0; i < CP_sets.size(); i++){
        ...
        if(cluster_fixed_nodes.size()>0){
            ...
            std::random_device rd;
            std::mt19937 gen(rd());
            std::uniform_int_distribution<> distrib1(0, cluster_graph.rows()-1);
            int node_id = distrib1(gen);
            
            std::uniform_int_distribution<> distrib2(0, num_fpgas-1);
            int fpga_id = distrib2(gen);
...
int main(int argc, char* argv[]) {
    ...
    std::string netlist_file_path = "./ICCAD2021-TopoPart-Benchmarks/generated_large_netlists/MFS2/" + data; ///Generated_Benchmarks/Generated_Benchmarks/  //generated_large_netlists/
    ...
    int capacity = ceil(7*netlist_matrix.rows()/fpga_matrix.rows());
